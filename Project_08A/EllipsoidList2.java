...
// ******NEW METHODS FOR PROJECT 6******
   ...
   */
   public Ellipsoid findEllipsoidWithSmallestVolume() {
      Ellipsoid[] elTest = new Ellipsoid[1];
      elTest[0] = ellipsoidlist[0];
      for (int i = 0; i < numberOfEllipsoid; i++) {
         if (ellipsoidlist[i].volume() < elTest[0].volume()) {
            elTest[0] = ellipsoidlist[i];
         }
      }
      return elTest[0];
   }     
   ...
   */ 
   public Ellipsoid findEllipsoidWithSmallestSurfaceArea() {
      ...
      elTest[0] = ellipsoidlist[0];
      for (int i = 0; i < numberOfEllipsoid; i++) {
         if (ellipsoidlist[i].surfaceArea() < elTest[0].surfaceArea()) {
            elTest[0] = ellipsoidlist[i];
         }
      }
      return elTest[0];
